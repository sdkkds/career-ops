...
- **run-nightly.ps1** — PS nightly orchestrator at project root; test with `-EvalOnly -MaxJobs 1 -DryRun` (smoke) or `-EvalOnly -MaxJobs 1` (live); Task Scheduler job `CareerOps-Nightly` runs daily at 6am; output goes to Vault `Fortress of Solitude\career-ops\`
